...
class MainPage extends Component {
    ...
    calculateFaceLocation(data) {
      ...
      updateRank = () => {
        api.get(
          `profile/${this.props.user.id}/rank`, 
          { headers: {'Authorization': `Bearer ${window.sessionStorage.getItem('token')}` }}
          )
        .then(data => {
    ...
    render () {
        const { imageURL, box, error, errorMessage, showPopup, entries } = this.state;
        return (
            <div>
                <Rank  className="mainpage" currentRank={entries || this.props.user.entries} name={this.props.user.name} />
                <ImageLinkForm onInputChange={this.onInputChange} onSubmit={this.onSubmit} onReset={this.onReset} />
                <FaceRecognition imageURL={error ? '' : imageURL} box={box}/>
                <legend className={error ? 'error center': 'invisible'}>{errorMessage}</legend>   
                { showPopup ?
