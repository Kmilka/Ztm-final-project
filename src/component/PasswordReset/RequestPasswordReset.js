...
class RequestPasswordReset extends Component {
  ...
  onSubmit = () => {
    if (this.isEmpty()) {
      ...
      api
        .post("password", { email: this.state.email })
        .then(
          this.setState({
      ...
      <div>
        {!this.state.requestSucceeded ? (
          ...
          <p className="white f3 pa3">
            A password reset link was sent to this email address:{" "}
            <span className="black f3 underline">{this.state.email}</span>.
            <br /> The link is active for 1 hour and for 1-time use only. If you
