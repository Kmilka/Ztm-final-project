...
class ConfirmPasswordResetComponent extends Component {
    ...
    componentDidMount() {
        api.get(`password/${this.props.token}`)
        .then(data => {
            this.setState({
                tokenIsValid: true,
                id: data.data
    ...
    onSubmit = () => {
        if (this.isEmpty()) {
            ...
            api.post('password/new', {id: this.state.id, password: this.state.password})
            .then(() => {
        ...
        return (
            <div>
                ...
                this.state.passwordChangeSucceeded ?
                    ...
                    <p className='white f3 pa3'>{this.state.errorMessage ? this.state.errorMessage : 'Your link has expired'}. Request a new one <Link to='/password-reset' className='underline link dim black'>here</Link></p>
                }
